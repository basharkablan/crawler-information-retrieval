...
def fetch_actor(actor_id):
    if have_actor(actor_id) == 0:
        ...
        data, movies_count = get_actor_movies(page)
        data.insert(0, actor_id)
        ws_actor_movies.append(data)
        save_actor(actor_id, actor_name, movies_count)
...
def fetch_movie(movie_id):
    if have_movie(movie_id) == 0:
        ...
        rating = get_movie_rating(page)
        ws_movies.append([movie_id, movie_name, rating])
...
def get_movie_rating(page):
    rating_div = page.find('span', itemprop='ratingValue')
...
def save_actor(actor_id, actor_name, movies_count):
    ws_actors.append([actor_id, actor_name, movies_count])
